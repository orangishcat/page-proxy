...
<aside class="flex w-56 shrink-0 flex-col border-l border-gray-700 bg-gray-950/35 p-4">
  ...
  {:else if activeStep?.kind === "select-element"}
    <div class="space-y-3">
      {#if selectElementCurrentSelector}
        <div class="rounded-md border border-gray-700 p-2">
          <p class="text-xs text-gray-500">Current selector</p>
          <p class="mt-1 font-mono text-xs text-gray-200 break-all">{selectElementCurrentSelector}</p>
        </div>
      {/if}
          ...
          <div class="space-y-2">
            {#each selectElementSelectorMatches as selectorMatch}
              <button
                ...
                title={selectorMatch}
              >
                <span class="font-mono text-xs text-accent-500 hover:text-accent-400">{truncate(selectorMatch)}</span>
              </button>
            {/each}
          </div>
        {/if}
