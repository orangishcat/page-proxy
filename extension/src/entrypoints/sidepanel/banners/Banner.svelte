<script lang="ts">
  ...
  const baseContainerClass = "flex w-full max-w-none shrink-0 items-center gap-2 px-4 py-2 text-caption";
  const baseDismissButtonClass = "rounded border px-2 py-0.5 text-caption";
...
<div class={`${baseContainerClass} ${variantClasses[variant].container} ${className}`}>
  ...
  {#if showDismiss}
    <button
      type="button"
      class={`${baseDismissButtonClass} ${variantClasses[variant].dismissButton} ${dismissButtonClass}`}
