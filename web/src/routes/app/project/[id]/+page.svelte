<script lang="ts">
  ...
  const toggleSelectionMode = () => {
    ...
    notifySelectionEnabled(selectionMode);
  };
  ...
  const notifySelectionEnabled = (enabled: boolean) => {
    if (childApi) {
      childApi.setSelectionEnabled(enabled);
      return;
    }
    if (!iframeEl || !iframeEl.contentWindow) return;
    iframeEl.contentWindow.postMessage(
      {source: messageSource, type: 'selection-enabled', enabled},
  ...
  const handleProxyMessage = (event: MessageEvent) => {
    ...
    if (data.type === 'hover') {
      hoveredInfo = data.payload as ElementInfo;
    }
    if (data.type === 'select') {
      selectedInfo = data.payload as ElementInfo;
    }
  };
